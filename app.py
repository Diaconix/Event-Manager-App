...
def admin_login():
    """Admin registration/login page"""
    st.title("🎪 Event Manager Pro")
    st.markdown("---")
    
    st.subheader("Create Your Event Space")
    
    with st.form("admin_setup"):
        admin_id = st.text_input("Your Organization Name *", 
                               placeholder="e.g., School_Event_Team, Company_Party_2024")
        admin_email = st.text_input("Contact Email (Optional)", 
                                  placeholder="for notifications")
        
        submitted = st.form_submit_button("Create My Event Space 🚀")
        ...
        if submitted:
            if admin_id:
                ...
                st.success(f"✅ Event space created for: {admin_id}")
                st.rerun()
            else:
...
def admin_dashboard():
    ...
    st.sidebar.info(f"Your isolated event space")
